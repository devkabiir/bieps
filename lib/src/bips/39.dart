...
class BIP39 {
  ...
  static List<String> generateMnemonics(Uint8List initialEntropy) {
    ...
    if (!(128 <= ent && ent <= 256 && ent % 32 == 0)) {
      throw new ArgumentError('initial entropy must be between 128-256 bits and multiple of 32');
    }

    /// checksum is generated by taking the first ent/32 bits of its sha256 hash
    final int cs = (ent ~/ 32);

    final Uint8List hash = new SHA256Digest().process(initialEntropy);
    ...
    return new RegExp(r'(.{1,11})') // Capture groups of 11 bits
        .allMatches(allBits) // from [allBits]
        .map<String>((Match m) => m.group(0)) // Convert the resulting list of bits
        .map<int>((String s) {
          if (s.length == 11) {
            return bitToByte(s);
          }
        ...
        .map<String>((int index) => _wordList[index]) // Get words from wordlist using these ints as index
        .toList();
  }
